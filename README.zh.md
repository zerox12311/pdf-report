...
在瀏覽器裡拖拉設計 PDF 樣板，送資料進來就產出 PDF。

這是一套開源的通用 PDF 產生器與樣板編輯器：設計人員在網頁編輯器裡排版面（文字、表格、條碼、圖片、浮水印），系統串接方只要把資料丟給它，就能拿到一份排好版的正式 PDF。報表、收據、標籤、證明文件……任何「固定版面＋變動資料」的文件都適用。

## 它能做什麼

- **像簡報軟體一樣設計版面**：拖拉元件、對齊、調字型字級，畫面上看到什麼，印出來就是什麼。
- **資料與版面分離**：版面上放的是「欄位」（例如客戶名稱、金額、日期），實際內容由送進來的資料決定。同一份樣板可以印出千百張不同內容的文件。
- **明細表格自動長**：一筆訂單有 3 行明細或 300 行都沒問題，表格自動換頁、每頁重畫表頭，還能做分組小計。
- **內建在地化格式**（需要才用）：金額國字大寫（壹拾貳萬…元整）、民國年日期、超商繳費三段條碼、中一刀／熱感紙張尺寸、「作廢」印章浮水印。
- **嵌進你自己的系統**：編輯器可以用 iframe 嵌入任何網頁系統，讓你的使用者在你的產品裡設計樣板；你的後端再呼叫 API 產出 PDF。
- **多人團隊管理**：內建登入、專案、成員權限，設計者各管各的樣板。

## 畫面

在編輯器裡設計樣板（拖拉元件、頁首頁尾、浮水印），範例為一張收款單：
