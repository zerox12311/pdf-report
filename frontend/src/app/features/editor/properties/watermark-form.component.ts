...
@Component({
  ...
  template: `
    <label class="full">{{ t('文字') }}
      <input [ngModel]="wm().text" (ngModelChange)="patchWm.emit({ text: $event })" [placeholder]="t('例：副本 / 作廢')" />
    </label>
    <label class="full">{{ t('依資料切換文字（選填）——例：綁 status，資料給「作廢」就蓋作廢') }}
      <input [ngModel]="wm().key" (ngModelChange)="patchWm.emit({ key: $event })" [placeholder]="t('例：watermark')" />
    </label>
    <app-slider-field [label]="t('字級（pt）')" [min]="12" [max]="200"
      [value]="wm().fontSize" (valueChange)="patchWm.emit({ fontSize: $event })" />
    <app-slider-field [label]="t('旋轉（度）')" [min]="0" [max]="360"
      [value]="wm().rotation" (valueChange)="patchWm.emit({ rotation: $event })" />
    <label>{{ t('顏色') }} <input type="color" [ngModel]="wm().color" (ngModelChange)="patchWm.emit({ color: $event })" /></label>
    <label class="full">{{ t('疊放位置') }}
      <select [ngModel]="wm().layer" (ngModelChange)="patchWm.emit({ layer: $event })">
        <option value="below">{{ t('蓋在內容下方（預設）') }}</option>
        <option value="above">{{ t('蓋在內容上方') }}</option>
      </select>
    </label>
    <label class="row"><input type="checkbox" [ngModel]="wm().repeat"
      (ngModelChange)="patchWm.emit({ repeat: $event })" /> {{ t('重複平鋪（鋪滿整頁）') }}</label>
    @if (wm().repeat) {
      <app-slider-field [label]="t('水平間隔（pt）')" [min]="10" [max]="400"
        [value]="wm().gapX" (valueChange)="patchWm.emit({ gapX: $event })" />
      <app-slider-field [label]="t('垂直間隔（pt）')" [min]="10" [max]="400"
